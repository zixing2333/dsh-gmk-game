...
export function apply(ctx) {
	...
	const css = `
		...
		.gomoku-plugin-grid-line-h { width: 100%; height: 1px; left: 0; }
		.gomoku-plugin-grid-line-v { height: 100%; width: 1px; top: 0; }
		.gomoku-plugin-stone {
			position: absolute;
			border-radius: 50%;
		...
		.gomoku-plugin-btn:active:not(:disabled) { transform: translateY(0); }
		.gomoku-plugin-btn:disabled { opacity: 0.4; cursor: not-allowed; }
		...
		@keyframes gomoku-plugin-fadein { from { opacity: 0; } to { opacity: 1; } }
		...
		.gomoku-plugin-win-restart-btn { max-width: 120px; flex: none; margin-top: 4px; }
		...
		.gomoku-plugin-minimized .gomoku-plugin-controls { display: none; }
		...
		.gomoku-plugin-last-marker-black { border: 2px solid rgba(255, 255, 255, 0.9); }
		.gomoku-plugin-last-marker-white { border: 2px solid rgba(50, 50, 50, 0.9); }
	`;
	const disposeStyle = ctx.styles.insert(css);

	const React = ctx.get("react");
	if (!React) return;
	const { useState, useEffect, useCallback, useMemo, createElement, Fragment } = React;
	...
	function GomokuGame() {
		...
		useEffect(() => {
			if (currentPlayer === 2 && !winner) {
				...
				const dispose = ctx.timer.timeout(() => {
		...
		}, [currentPlayer, winner, board, makeAIMove, checkWin]);
